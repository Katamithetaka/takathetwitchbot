...
const OAUTH_TOKEN = twitch.access_token; // Needs scopes user:bot, user:read:chat, user:write:chat
...
async function getAuth() {
    ...
    const user_response = await fetch("https://api.twitch.tv/helix/users?login=" + twitch.channel_name, {
        method: "GET", 
        headers: {
		...
		case 'notification': // An EventSub notification has occurred, such as channel.chat.message
			switch (data.metadata.subscription_type) {
				case 'channel.chat.message':
					if(data.payload.event.chatter_user_id == CHAT_CHANNEL_USER_ID && data.payload.event.message.text.includes("!reset")) {
						await reset();
					}
					break;
				case 'channel.channel_points_custom_reward_redemption.add':
                    ...
                    else if(data.payload.event.reward.id == twitch.counterclockwise_event) {
