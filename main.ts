...
const { sceneItemTransform } = await obsClient.call(
...
export async function rotate(unit: number) {
  ...
  const r = {
    top_left: [
      Math.abs(rectangle.top_left[0] / w),
      Math.abs(rectangle.top_left[1] / h),
    ],
    top_right: [
      Math.abs(rectangle.top_right[0] / w),
      Math.abs(rectangle.top_right[1] / h),
    ],
    bottom_left: [
      Math.abs(rectangle.bottom_left[0] / w),
      Math.abs(rectangle.bottom_left[1] / h),
    ],
    bottom_right: [
      Math.abs(rectangle.bottom_right[0] / w),
      Math.abs(rectangle.bottom_right[1] / h),
    ],
  };
...
/// ASSUMES OBS IS RUNNING IN 1920x1080!!!!!
const rect = {
  top_left: [-960, -540],
  top_right: [960, -540],
  bottom_left: [-960, 540],
...
function rotate_by_angle(angle: number) {
  return {
    ...
    bottom_left: [
      rect.bottom_left[0] * Math.cos(angle) -
      rect.bottom_left[1] * Math.sin(angle),
      rect.bottom_left[0] * Math.sin(angle) +
      rect.bottom_left[1] * Math.cos(angle),
    ],
    bottom_right: [
      rect.bottom_right[0] * Math.cos(angle) -
      rect.bottom_right[1] * Math.sin(angle),
      rect.bottom_right[0] * Math.sin(angle) +
      rect.bottom_right[1] * Math.cos(angle),
    ],
  };
}
